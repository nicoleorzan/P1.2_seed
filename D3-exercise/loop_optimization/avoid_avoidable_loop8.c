...
int main(int argc, char **argv)
{
  ...
  // ---------------------------------------------------
  // STEP 8: let the compiler vectorize, first attempt
  // ---------------------------------------------------
  
  printf("LOOP 8 :: "); fflush(stdout);
...
#pragma ivdep    	    
	      for(k = 0; k < Ng; k+=4)
		// PLS NOTE:: for simplicity we assume that Np % 4 = 0
		{
		  /* double dz2[4] = {(parts[p+2] - jks[k])*(parts[p+2] - jks[k]), */
		  /* 		   (parts[p+2] - jks[k+1])*(parts[p+2] - jks[k+1]), */
		  /* 		   (parts[p+2] - jks[k+2])*(parts[p+2] - jks[k+2]), */
		  /* 		   (parts[p+2] - jks[k+3])*(parts[p+2] - jks[k+3]) }; */
